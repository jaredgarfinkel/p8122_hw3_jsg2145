...
set.seed(719)
training_dag = dagify(re78 ~ treat + re74 + re75 + nodegree + age + black + hispan,
                      treat ~ educ + married,
                      re74 ~ nodegree + age + black + hispan,
                      re75 ~ nodegree + age + black + hispan,
                      nodegree ~ educ,
                      married ~ age,
...
```{r}
subclass.breaks = quantile(ps2, c(.33, .67)) # bins (initial try - modify as needed)
subclass = ps2
subclass = as.numeric(ps2>subclass.breaks[1])
subclass[which(ps2>subclass.breaks[1]& ps2<=subclass.breaks[2])]<- 1
...
Even with 3 subclasses there is a subclass that is small.
...
prop.func <- function(x, trt) {
  ...
  ps2[which(ps2>subclass.breaks[2]&ps2<=subclass.breaks[3])]
}
data2$ps <- ps2
check.overlap(x = data2[which(ps2>subclass.breaks[2]&ps2<=subclass.breaks[3]),],
              trt = data2$treat[which(ps2>subclass.breaks[2]&ps2<=subclass.breaks[3])],
              type = "both",
              propensity.func = prop.func)
